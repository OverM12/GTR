...
import ApexLineChart from "@/app/environment/ApexLineChart";
    ...
    const fetchGtrData = async () => {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          const savedNotes = localStorage.getItem('userEnvironmentNotes');
          if (savedNotes) {
        ...
        // Update environmentData state
        const environmentDataResponse = data.data.data.areas.environment;
        setEnvironmentData(environmentDataResponse);

        if (environmentDataResponse && environmentDataResponse.gtr) {
          setGtrScore(parseFloat(environmentDataResponse.gtr));
        } else {
          setGtrScore(0);
        }

        // Only update reflectionText from API if no saved notes from localStorage or if API has newer data
        if (data.data.data.environmentNotes && !localStorage.getItem('userEnvironmentNotes')) {
          setReflectionText(data.data.data.environmentNotes);
        } else if (!data.data.data.environmentNotes && !localStorage.getItem('userEnvironmentNotes')) {
          setReflectionText("");
        }
  ...
  const handleEditSaveToggle = async () => {
    if (isEditingReflection) {
      ...
      try {
        setLoading(true);
        if (typeof window !== 'undefined') {
          localStorage.setItem('userEnvironmentNotes', tempReflectionText);
        }
  ...
  return (
    <div className="w-full flex flex-col bg-[#F0F2F5] py-[32px] px-[16px] gap-[16px]">
      ...
      <div className="w-full flex flex-col bg-white p-6 rounded-4xl">
        <div className="mb-6">
          <h2 className="font-semibold text-[24px]">Your personal Obtainment reflection notes</h2>
