  ...
  useEffect(() => {
    const fetchGtrData = async () => {
      if (!dateRange.fromDate || !dateRange.toDate) {
        //console.log("environment page: Date range not complete, using default 0");
        setGtrScore(0);
      ...
      try {
        setLoading(true);
        //console.log("environment page: Fetching data for date range:", dateRange);
        const data = await reportService.getGtrReport(dateRange.fromDate, dateRange.toDate);
        //console.log("environment page: Data fetched successfully:", data);

        // Check for environment data in the response
        const environmentData = data.data.data.areas.environment;
        if (environmentData && environmentData.gtr) {
          setGtrScore(parseFloat(environmentData.gtr));
        } else {
          setGtrScore(0); // Default to 0 if gtr score is not available
        }

        // Handle the reflection text or any other data
        if (environmentData && environmentData.reflection) {
          setReflectionText(environmentData.reflection);
        } else {
  ...
  return (
    <div className="w-full h-lvh overflow-auto flex flex-col bg-[#F0F2F5] py-[32px] px-[16px] gap-[16px]">
      <h1 className="text-[#737985] text-[24px]">
        Insights / <strong className="text-black">Environment</strong>
      </h1>
      ...
      <div className="flex flex-col bg-white p-4 rounded-4xl">
        <div className="reflection-content">
          <h3 className="font-semibold mb-2">
            Your personal environment reflection notes
          </h3>
          ...
          <div className="w-full bg-[#F0F2F5] rounded-[24px] p-[32px] flex flex-col justify-center">
            <h1 className="font-bold">Reflection on my current environment</h1>
          ...
          >
            <Image
              ...
              width={20}
              height={20}
              src="/your-gtr/your-gtr/environment-insights/edit-icon.svg"
            />
            Edit reflection
          </button>
        </div>
