...
import ApexLineChart from "@/app/actions/ApexLineChart";
  ...
  useEffect(() => {
    const fetchGtrData = async () => {
      ...
      try {
        ...
        if (actionsDataResponse && actionsDataResponse.gtr) {
          setGtrScore(parseFloat(actionsDataResponse.gtr));
        } else {
          setGtrScore(0);
        }
  ...
  return (
    <div className="w-full flex flex-col bg-[#F0F2F5] py-[32px] px-[16px] gap-[16px]">
      ...
      <div className="w-full flex flex-col bg-white p-6 rounded-4xl">
        <div className="mb-6">
          <h2 className="font-semibold text-[24px]">Your personal Action reflection notes</h2>
