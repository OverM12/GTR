    ...
    const fetchGtrData = async () => {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          const savedNotes = localStorage.getItem('userGetsNotes');
          if (savedNotes) {
        ...
        if (data.data.data.getsNotes && !localStorage.getItem('userGetsNotes')) {
          setReflectionText(data.data.data.getsNotes);
        } else if (!data.data.data.getsNotes && !localStorage.getItem('userGetsNotes')) {
          setReflectionText("");
        }
  ...
  const handleEditSaveToggle = async () => {
    if (isEditingReflection) {
      ...
      try {
        setLoading(true);
        if (typeof window !== 'undefined') {
          localStorage.setItem('userGetsNotes', tempReflectionText);
        }
        ...
        {loading ? (
          ...
          <div className="flex w-full items-center hover:bg-[#F0F1F5] py-6 rounded-[24px]">
            <div className="flex items-center gap-2 pl-2">
              {/* <Image
                src="/your-gtr/dashboard/gets-icon.png"
                width={40}
                height={40}
                alt="Gets Icon"
              />
