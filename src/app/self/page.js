    ...
    const fetchGtrData = async () => {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          const savedNotes = localStorage.getItem('userReflectionNotes');
          if (savedNotes) {
        ...
        if (data.data.data.selfNotes && !localStorage.getItem('userReflectionNotes')) {
          setReflectionText(data.data.data.selfNotes);
        } else if (!data.data.data.selfNotes && !localStorage.getItem('userReflectionNotes')) {
          setReflectionText("");
        }
  ...
  const handleEditSaveToggle = async () => {
    if (isEditingReflection) {
      ...
      try {
        setLoading(true);
        if (typeof window !== 'undefined') {
          localStorage.setItem('userReflectionNotes', tempReflectionText);
        }
    ...
    <div className="w-full flex flex-col bg-[#F0F2F5] py-[32px] px-[16px] gap-[16px]">
      ...
      <div className="w-full flex flex-col bg-white p-2 rounded-4xl py-6 px-6">
        ...
        {loading ? (
          ...
          <div className="flex w-full items-center py-6 rounded-[24px]">
      ...
      <div className="w-full flex flex-col bg-white p-6 rounded-4xl">
        <div className="mb-6">
          <h2 className="font-semibold text-[24px]">Your personal Self reflection notes</h2>
